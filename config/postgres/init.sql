...
(2, 'Che differenza c''è tra le istruzioni ENTRYPOINT e CMD in un Dockerfile?', '["ENTRYPOINT definisce l''eseguibile fisso del container; CMD definisce i parametri di default sovrascrivibili", "CMD definisce l''eseguibile fisso; ENTRYPOINT definisce le variabili d''ambiente", "Sono perfettamente identiche e intercambiabili", "ENTRYPOINT serve solo in fase di build, CMD solo in esecuzione"]', 0, 'ENTRYPOINT stabilisce il comando principale ed immutabile che verrà avviato nel container. CMD stabilisce gli argomenti di default passati a ENTRYPOINT, che l''utente può sovrascrivere all''avvio.'),
